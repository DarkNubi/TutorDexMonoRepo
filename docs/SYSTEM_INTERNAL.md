...
- Side-effect defaults: if `ENABLE_BROADCAST` is not set, the worker auto-enables broadcasts when a bot token/API URL and `AGGREGATOR_CHANNEL_ID`/`AGGREGATOR_CHANNEL_IDS` are configured; if `ENABLE_DMS` is not set, `DM_ENABLED=true` will enable DMs.
...
- GitHub Actions store secrets for:
  - Tailscale deploy: `TS_OAUTH_CLIENT_ID`, `TS_OAUTH_SECRET`, `SERVER_TS_IP`, `SERVER_USER`, `SERVER_SSH_KEY` (the workflow uses the `tag:github-autodeploy-ci` runner identity)
