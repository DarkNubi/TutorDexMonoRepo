...
- `deploy.yml`: runs automatically on pushes to `main`; authenticates the GitHub runner with the Tailscale OAuth trust credential and `tag:github-autodeploy-ci`, connects to the Windows server through Tailscale SSH, pulls the latest code under `D:/TutorDex`, then runs `docker compose up -d --build --pull=never`.
