...
def setup_sentry(*, service_name: str = "tutordex-aggregator") -> None:
    ...
    environment = str(cfg.sentry_environment or "development").strip()
    # In fast-moving development, using `EXTRACTION_PIPELINE_VERSION` as a release placeholder is
    # acceptable for grouping, but in production you should prefer an immutable build identifier.
    release = str(cfg.sentry_release or "").strip() or str(cfg.extraction_pipeline_version or "").strip() or None
