...
def upsert_broadcast_message(*, payload: Dict[str, Any], sent_chat_id: int, sent_message_id: int, message_html: str) -> bool:
    """Store broadcast message mapping for tracking and reconciliation."""
    cfg = load_config_from_env()
    if not _enabled(cfg):
        logger.debug("broadcast_tracking_disabled: Supabase not enabled")
        return False

    original_url = str(payload.get("message_link") or "").strip()
    if not original_url:
        logger.debug("broadcast_tracking_skipped: No message_link in payload")
        return False

    external_id = _derive_external_id(payload)
    if not external_id:
        logger.debug("broadcast_tracking_skipped: No external_id")
        return False
    ...
    try:
        ...
        logger.debug("assignment_clicks_upsert_failed: %s", e)
